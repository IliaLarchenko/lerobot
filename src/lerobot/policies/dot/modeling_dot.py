...
class DOT(nn.Module):
    def __init__(self, config: DOTConfig):
        ...
        if len(self.image_names) > 0:
            ...
            backbone.fc = nn.Linear(backbone.fc.in_features, self.config.dim_model, bias=self.config.projections_bias)
        ...
        if self.config.robot_state_feature:
            self.projections["state"] = nn.Linear(
                self.config.robot_state_feature.shape[0], self.config.dim_model, bias=self.config.projections_bias
            )
            self.n_features += self.config.n_obs_steps

        if self.config.env_state_feature:
            self.projections["environment_state"] = nn.Linear(
                self.config.env_state_feature.shape[0], self.config.dim_model, bias=self.config.projections_bias
            )
            self.n_features += self.config.n_obs_steps
...
class DOTPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        action = self._chunk_actions(actions)
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        if len(self.config.image_features) > 0:
            ...
            batch[OBS_IMAGES] = torch.stack([batch[k] for k in self.image_names], dim=2).flatten(
                1, 2
            )  # bs, n_obs * n_cam, c, h, w
